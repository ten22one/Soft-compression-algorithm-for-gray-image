...
import PreProcess_s
import ShapeFinding_s
import CodeProcessing_s
import Encode_s
import Decode_s
import os
import numpy as np
...
if __name__ == '__main__':
    ...
    PreProcess_s.dir_check(frequency_dir, empty_flag=True)
    PreProcess_s.dir_check(codebook_dir, empty_flag=True)
    PreProcess_s.dir_check(output_dir[0], empty_flag=True)
    PreProcess_s.dir_check(output_dir[1], empty_flag=True)
    # Shape searching
    for f in os.listdir(input_dir[0]):
        ...
        ShapeFinding_s.ShapeFinding(process_dir, after_process_dir, layer_shape, shape_height, shape_width, rough_height,
                                    rough_width, start)
    # Codebook processing
    for f in os.listdir(frequency_dir):
        ...
        CodeProcessing_s.CodeProcessing(process_frequency, after_process_dir)
    ...
    for f in os.listdir(codebook_dir):
        ...
        for g in os.listdir(input_dir[1]):
            ...
            rate = Encode_s.Encode(codebook_use_dir, encode_dir, test_encode_dir, pixel_size, layer_shape, rough_height,
                                   rough_width, start)
    ...
    for f in os.listdir(output_dir[0]):
        ...
        for g in os.listdir(os.path.join(output_dir[0], f)):
            ...
            rate = Decode_s.Decode(codebook_use_dir, decode_dir, test_decode_dir, original_img_dir, start)
            error_rate[int(f)][int(g)] = rate
