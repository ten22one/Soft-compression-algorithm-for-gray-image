...
class ArithmeticEncoder(ArithmeticCoderBase):
    ...
    def finish(self):
        self.output.write(1)
        while self.output.numbitsfilled != 0:
            self.output.write(0)
...
class BitInputStream:
    ...
    def read(self):
        # print('self.currentbyte', self.currentbyte)
        if self.currentbyte == -1:
            return -1
        if self.numbitsremaining == 0:
            # print("hello",self.input[self.number:self.number + 8])
            if self.number == len(self.input):
                self.currentbyte = -1
                return -1
            temp_int = int(self.input[self.number:self.number + 8], 2)
            self.number += 8
            temp = bytes((temp_int,))
...
class BitOutputStream:
    ...
    def __init__(self):
        self.output = ''  # The underlying byte stream to write to
    ...
    def write(self, b):
        ...
        if self.numbitsfilled == 8:
            ...
            w = format(self.currentbyte, 'b').zfill(8)
            self.binary.append(format(self.currentbyte, 'b').zfill(8))
